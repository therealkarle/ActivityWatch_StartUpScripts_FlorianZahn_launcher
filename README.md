...
- Ein `activityWatchCheck`-Block kann `url` und `retryDelaySeconds` direkt enthalten. Alternativ akzeptiert der Launcher auch ein eingebettetes `activityWatch`-Objekt mit denselben Feldern.
...
- Der `activityWatchCheck`-Block kann an beliebiger Stelle in `blocks` eingefuegt werden, um die Warteposition im Ablauf zu steuern. Wenn ActivityWatch noch nicht online ist, wartet der Launcher `retryDelaySeconds` Sekunden und probiert es erneut, bis die Instanz antwortet.
